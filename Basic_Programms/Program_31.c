/*
    Algorithm

    START
        Accept a number from the user as iNum
        Apply filters in main for negative, zero, and one
        If number is greater than one:
            Initialize iCnt to 2
            Use a while loop to check divisibility from 2 up to (iNo - 1)
            If iNo is divisible by iCnt, return false immediately
            Increment iCnt in each iteration
            If the loop completes without finding a factor, return true
        Display whether the number is prime based on the boolean result
    STOP
*/

/////////////////////////////////////////////////////////////////
// 
//  Required Header files
//
/////////////////////////////////////////////////////////////////

#include<stdio.h>
#include<stdbool.h>

/////////////////////////////////////////////////////////////////
//
//  Function Name : CheckPrime
//  Description :   It is used to check whether a number is prime using while loop         
//  Input :         Integer
//  Output :        Boolean (bool)
//  Author :        Tushar vikas bhore
//  Date :          04/03/2026
//
/////////////////////////////////////////////////////////////////

bool CheckPrime( int iNo )
{

    int iCnt = 0;                                    // Loop counter

    iCnt = 2;                                        // Initialization

    while( iCnt < iNo )                              // Loop condition
    {

        if( iNo % iCnt == 0 )                        // Business Logic: Check divisibility
        {

            return false;                            // Factor found, not prime
            break;                                   // Unreachable code
        
        }

        iCnt++;                                      // Increment counter

    }

    return true;                                     // No factors found, it is prime
    
}   // End of CheckPrime

/////////////////////////////////////////////////////////////////
//
//  Entry point function for the application
//
/////////////////////////////////////////////////////////////////

int main()
{

    int iNum = 0;                                    // To store user input
    bool bRet = false;                               // To store result

    printf( "Enter the number :\t" );
    scanf( "%d" ,&iNum );

    if( iNum < 0 )                                   // Input Validator
    {

        printf( "Error : Please enter the positive numbers only.\n" );
        return -1;

    }
    else if( iNum == 0 )                             // Filter for zero
    {

        printf( "The Zero is not prime.\n" );
        return -1;

    }
    else if( iNum == 1 )                             // Filter for one
    {

        printf( "The number one is nor prime nor composite.\n" );
        return -1;

    }

    bRet = CheckPrime( iNum );                       // Function Call

    if( bRet == true )                               // Result check
    {

        printf( "The number is prime.\n" );

    }
    else
    {

        printf( "The number is not prime.\n" );

    }

}   // End of main

/////////////////////////////////////////////////////////////////
//
//  Testcases successfully handled by the application
//
//  Input : 11       Output : The number is prime.
//  Input : 21       Output : The number is not prime.
//  Input : 2        Output : The number is prime.
//  Input : -7       Output : Error : Please enter...
//
/////////////////////////////////////////////////////////////////